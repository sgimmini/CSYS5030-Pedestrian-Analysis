...
################################################## 
# MI
weekdays <- read.csv2("pedestrians/results/weekdays/weekday_hourly_MI_TL5.csv", sep = ",")
weekdays <- read.csv2("pedestrians/results/weekdays/weekday_hourly_MI_TL5_stat_sig.csv", sep = ",")
...
plot_MI_heatmap(weekdays, "weekdays_MI_heatmap.pdf")

colfunc<-colorRampPalette(c("white","yellow","red","black"))

max_tl <- max(weekdays$Time_lag)

max_MI <- as.numeric(max(as.numeric(weekdays$MI)))
min_MI <- as.numeric(min(as.numeric(weekdays$MI)))
...
################################################## 
################################################## 
# AIS comparison between splitting and not splitting
...
weekday_AIS_wrong <-  read.csv2("pedestrians/results/weekdays/weekday_hourly_AIS_unsplit_stat_sig.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)

# join both data frames to have columns AIS and AIS_wrong 
weekday_AIS %>% 
  left_join(weekday_AIS_wrong, by = c("Sensor", "Year", "Month", "Day")) %>% 
  rename(Stat_Sig = Stat_Sig.x, Stat_Sig_wrong = Stat_Sig.y) %>% 
  rename(AIS = AIS.x, AIS_wrong = AIS.y) %>%
  mutate(Month = 1) %>% 
  unite(Date, Year, Month, Day, sep = "-") %>%
  mutate(Date = as.Date(Date), AIS = as.numeric(AIS), AIS_wrong = as.numeric(AIS_wrong)) %>%
  mutate(Difference = abs(AIS - AIS_wrong)) -> weekday_AIS_diff

weekday_AIS_diff %>% 
  # get top 5 sensors with highest difference per day
  group_by(Date) %>%
...
weekday_AIS_diff %>%   
  filter(Sensor %in% diff_sensors$Sensor) %>% 
  # plot
  ggplot(aes(x = Date, y = Difference, color = factor(Sensor))) +
  geom_line() +
  labs(x = "Date", y = "Difference in AIS", color = "Sensor") +
  scale_color_manual(values = our_colors) 
...
weekday_AIS %>% 
  ...
  geom_line() + geom_point(aes(size = Stat_Sig)) + 
...
# save
ggsave("weekday_AIS_2018_min_max.pdf", plot=plot, width = 10, height = 5, units = "in", dpi = 300, limitsize = FALSE)
